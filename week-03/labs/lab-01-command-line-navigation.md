...
```
The 'grep' command searches for specific text in files
```
...
```
The path '/home/morgan' is written in Linux style. A way to tell that the path is in Linux style are the forward slashes in the path. Another way to tell is that the root drive doesn't have a letter.
```
...
```
The move [cd], then check [pwd] habit matters because you always want to confirm that you're in the correct folder/directory. Moving then checking right after will catch a wrong directory change and avoid any command mishaps. If two files have the same name, but are in two different directories, you want to make sure you have the right file by making sure you're in the correct directory, especially before making any changes to the file.
```
...
```
Asking the terminal for help instead of memorizing everything in advance matters for a real career in cybersecurity because knowing your resources and where to look for things will save a lot of time and help resolve issues more quickly than trying to go off of memory alone. There are tons of commands so referring to [help] or [man] to figure out what a command means is a useful tool. It's better to ask the terminal for help rather than Google or a search engine because you can get conflicting results on what the command actually does.
```
...
```
Nothing really surprised me, but using the command line always feels different from GUI since I rarely practice with the CLI. I have some exposure to the CLI through CTFs, so it wasn't completely new to me (thank goodness). The labeled cabinets analogy made the directories in Linux easier for me to understand.
```
...
```markdown
https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-session-1.png
https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-session-2.png
```
