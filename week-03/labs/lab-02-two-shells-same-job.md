...
```
New-Item -Item Type File -Path investigation-notes.txt
```
...
| 5. Create + back up your note | touch investigation-notes.txt and cp investigation-notes.txt investigation-notes-backup.txt | New-Item -Item Type File -Path investigation-notes.txt and Copy-Item investigation-notes.txt investigation-notes-backup.txt |
...
```
The information listed for the [ls] command looks different in Bash vs. PowerShell. In Bash, the files and folders are listed with just the file or folder name. In PowerShell the [ls] command shows the files/folders listed in columns (Mode and Name).
```
...
```
I knew the underlying file system was identical on both Bash and PowerShell by verifying the root location at the beginning with the [pwd] and [Get-Location] commands. They both gave the same output for the path '/home/agent'. Using the [ls] and [dir] commands showed the same file and folder, 'archive' and 'README.txt'. Drilling deeper, the 'archive' folder in both systems showed 'incident-07' and 'incident-42' folders. They both had identical folders and file paths.
```
...
```
The Bash (Linux) commands felt more intuitive to me because I've had some experience using Linux commands. The PowerShell commands felt different because the commands are a bit longer for some things and I don't really use the Windows terminal very often. It's easier for me to remember the short Linux commands over the longer Windows commands.
```
...
```
The differences between the systems weren't really visible by the slash styles, case-sensitivity, or drive letters because the slash styles in both systems showed forward slashes, all the files/folders were lowercase in both systems, and there wasn't a drive letter shown in the PowerShell. Two ways I noticed I was in Bash was because of the prompt having 'lnx01' and a '$' in the name (agent@storeroom-lnx01:/home/agent$) and PowerShell commands would give me an error message if I used them in Bash. I could tell I was in PowerShell based on 'PS' and '>' in the prompt name (PS /home/agent>) and the Windows commands executing properly.
```
...
![CLI Simulator Bash Session 2 Screenshot 1](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-bash-session-2-pic1.png)
![CLI Simulator Bash Session 2 Screenshot 2](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-bash-session-2-pic2.png)
![CLI Simulator PS Session 2 Screenshot 1](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-ps-session-2-pic1.png)
![CLI Simulator PS Session 2 Screenshot 2](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-03/cli-simulator-ps-session-2-pic2.png)
