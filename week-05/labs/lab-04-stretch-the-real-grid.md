...
```
My IPv4 address is 10.0.0.181. My default gateway is 10.0.0.1
```
...
IPv6 gateway: fe80::3e04:61ff:fee0:cdc
```

Which of the addresses you found is the one your computer actually uses to reach the internet, and how did you decide?

```
My computer uses 10.0.0.1 to reach the internet. I decided this because the Network settings said that's my router's IP address.
```
...
```
Yes, my IPv4 address falls into the private range, it starts with 10. Having a private address that doesn't reach the outside world tells me that there must be another IPv4 address for my public address.
```
...
```
1  docsis-gateway (10.0.0.1). This is the gateway to the outside network.
```
...
```
1st hop had 3 different times: 26.458 ms  4.648 ms  3.729 ms
The last hop just showed stars: ***
```
...
```
I see a pattern of the time increasing overall with each hop. The hops that provide info show distances traveled in my state. Looks like it hopped from a few different cities and then it was all **** after the third city.
```
...
```
Yes. After hop 8, hops 9 - 64 all were ***.
```

In one or two sentences: what do those stars tell you, and what do they specifically *not* tell you?

```
The stars tell me that some operators didn't respond to my trace request, which is normal. The stars do not mean that my request failed. It doesn't tell me that the site is up or down.
```
...
```
Two addresses came back: 162.159.130.67 and 162.159.129.67
```
...
```
One reason a large service may wan to use several addresses for one website is to have a backup server in case the main server goes down. Another reason they may want to have several addresses is to have different addresses in different geographical areas, that way users can connect to the IP address/server that is closest to them for a faster connection.
```
...
```
My public IP address, my computer's name, my username in the terminal, my ISP name, and server addresses.
```
...
```
I used the annotate rectangle method in macOS Preview. I covered my public IP address, my ISP name, my computer's name, my terminal's name, and the addresses of the servers for each hop that had information.
```
...
```
I removed the locations of the hops from questions 3 and 4.
```
...
```
My real output was more complicated than 10.20.5.42 because I had to go to my network settings to find my gateway, the [ip route] command wasn't populating anything in my terminal. Pinging my gateway just kept going because I forgot to ad [-c 4] to the command so it would stop at 4 packets. The [traceroute] command took a long time since I picked a server based in Europe. A teaching environments is built tidy so that we don't get overwhelmed looking at all the information a CLI can show from one command. It allows us to get used to knowing what to look for without having to search through a lot of data and getting confused.
```
...
```
Traceroute can tell me my private/public IP address, my ISP, the location of the hops and the time it took for the packet to travel. It'll also show *** when an operator doesn't respond to my request due to security reasons or it just doesn't want to reply. The stars do not mean that my request failed, my packet was still sent to the next hop on the route. The traceroute showing *** doesn't tell me whether or not the site is running or has crashed.
```
...
```
I choose to hide my public IP address, my ISP name, my computer's name, my terminal's name, and the addresses of the servers for each hop that had information. The instructions didn't specifically mention redacting the IP address shown by the servers, but I decided to cover those as well because they are linked to specific locations. I left my private IP address visible, because that can't be traced directly back to me. I left the times that the packets traveled visible. I deliberately left my private IP address and the travel times visible because that information isn't pertinent so it's okay if that's visible to the public. Any information that could give out my location, I redacted for security purposes. 
```
...
```
stretch-real-traceroute-1.png
stretch-real-traceroute-2.png
```
