...
```text
When data is encrypted, I expect to see hexadecimal numbers or some characters that don't make sense when I try to read it. Encrypted data will not look like the original report because it has been transformed using a mathematical equation. The data is changed from plaintext or readable text into cipher-text.
```
...
```text
This lab demonstrates confidentiality because I encrypted a plaintext file so that it was no longer readable. That shows that unauthorized users who don't have the key will not be able to read that file. Encryption does not prevent deletion because I could still run a command to delete the encrypted incident report file.
```
