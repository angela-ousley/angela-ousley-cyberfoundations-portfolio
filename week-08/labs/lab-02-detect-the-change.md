...
```text
If one line is added to a file, its SHA-256 digest will not remain the same. If any changes are made to a file, even just an added space or a deletion of one letter, the hash value will change.
```
...
```text
The mismatch proves that the hashes of the two files don't match, which means the two files are not identical. The mismatch does not prove who made the change to the file. It also doesn't prove that the file is safe of malware.
```
