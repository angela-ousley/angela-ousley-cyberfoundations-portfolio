...
```text
I predict that the 250 deny rule will block the incoming traffic because it comes before the 300 allow rule. The 300 priority rule will not get to be evaluated.
```
...
```text
My diagnosis is that the Deny rule is working as expected. Evidence that rules out my VM stopped: 

1. VM status says 'running'
2. The Python listener is listening in the VM, 'Serving HTTP on 0.0.0.0 port 8080' 
3. My original Allow rule is still active
```
...
```text
Grid Beacon verdict: Allowed
Other Test Source verdict: Denied
```
...
![Broken rules — week07-lab05-broken-rules.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab05-broken-rules.png)

![Observed denial — week07-lab05-observed-denial.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab05-observed-denial.png)

![Fixed rules — week07-lab05-fixed-rules.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab05-fixed-rules.png)

![Retest results — week07-lab05-retest-results.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab05-retest-results.png)
...
```text
Diagnosing from evaluation order is better than changing rules by trial and error because you never want to change a rule without understanding what you're changing and why you're changing it. Checking each rule in order also ensures you're changing a rule at the right priority level. If you diagnose from a higher priority number, you can' properly pinpoint what the issue is if there's a rule with a lower priority number that cancels out the one you are changing. Always evaluate in the order that the firewall/NSG follows to find the root cause.
```

**Analysis Question 3.** What made this failure safe and recoverable in the course environment? (Minimum 3 sentences.)

```text
This failure was safe and recoverable in the course environment because the Deny rule that created the failure was made by me. I added it to the editable rule priorities (200–999) by assigning it number 250. I was able to add it, which created the failure and I was also able to delete it which is how I recovered from the failure. I made sure not to edit or add to any of the protected rules.
```
