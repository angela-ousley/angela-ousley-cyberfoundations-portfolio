...
```text
1. Grid Beacon should receive an 'allow' verdict while no student Allow rule exists because there's no rule saying to deny all inbound traffic if it doesn't have a specific allow rule.

2. The Grid Beacon should receive an 'allow' verdict after the narrow Allow is created because the allow rule list the IP address (10.60.6.4) of the Beacon specifically.
```
...
Source: 10.60.6.4
Source port: *
Destination: 10.60.6.35
Destination port: 8080
Note: Week 7 least-privilege test service
```
...
![Completed narrow rule — week07-lab03-rule-created.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab03-rule-created.png)

![Grid Beacon ALLOWED result — week07-lab03-beacon-allowed.png](https://raw.githubusercontent.com/angela-ousley/angela-ousley-cyberfoundations-portfolio/refs/heads/main/assets/screenshots/week-07/week07-lab03-beacon-allowed.png)
...
```text
Allowing one source to one TCP port is narrower than allowing a whole subnet or any source because you're denying the majority of inbound traffic with that one rule. Allowing 'any' source lets all inbound traffic come to my VM. That's a very risky setting to have. Allowing a whole subnet narrows down the inbound traffic to however many devices that live on that subnet, could be a few or it could be thousands of addresses allowed to send data to my VM. Allowing one specific IP address to one specific port is the most narrow rule out of the three.
```
...
```text
The Python service is used to listen for connections and the Network Security Group (NSG) is used to set rules. Using python3 in the SSH listens for incoming network connections on a specific port, that's how I could tell if the Beacon test was connecting to my VM. The NSG controls the inbound and outbound rules, it determines what is allowed or denied.
```

**Analysis Question 2.** Why is `10.60.6.4` a stronger source value than Any for this task? (Minimum 4 sentences.)

```text
10.60.6.6 is a stronger source value than 'any' for security purposes because it applies the principal of least privilege. The purpose of the task is to see if a rule works as expected and to test the connection. Various sources being able to make a connection isn't necessary. Outside traffic being able to connect to my VM through different ports isn't necessary either, so narrowing down the traffic to one source IP address and one destination port is all that's needed to test student rules.
```

**Analysis Question 3.** What does an `ALLOWED` result prove, and what does it not prove about other sources? (Minimum 3 sentences.)

```text
An 'allowed' result proves that the inbound connection was approved and successfully connected to my VM. It does not prove that other IP addresses or ports are allowed or denied. A different IP address may be able to send me inbound traffic to a port other than 8080 on my VM.
```
